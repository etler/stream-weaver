...
Modern streaming architectures face a coordination challenge when processing dynamically expanding work while maintaining output order. Existing approaches typically require either pre-planning streaming pipeline work or implementing complex runtime coordination mechanisms. This paper presents Stream Weavers, a streaming framework pattern that uses async generators with unresolved tail promises to create unbounded async iterator chains. This approach enables constructing chains of streams that can be dynamically allocated at runtime, allowing the coordination of parallel work that may discover additional work during execution while preserving output sequence order. We describe the framework's core primitives and demonstrate their application in two domains: AI agent orchestration for parallel recursive content generation, and isomorphic UI rendering with server-client coordination. The framework provides an alternative approach to stream coordination by guaranteeing sequntial consumption of parallel streams with simplified orchestration requirements.
